...
Autoplay is muted and inline, and native looping remains enabled. The frame overlays simple controls: a play/pause toggle and a seek bar, always visible on touch devices and revealed on hover, focus, or pause elsewhere. Pausing through the toggle is respected until the visitor plays again. Browsers may still suspend playback or require a user gesture. The player resumes on focus, visibility, and page restoration, and exposes a Play demo button if paused outside the toggle. Failed requests can be retried. Unsupported MP4 and runtime decoding failures can fall back to WebM.
