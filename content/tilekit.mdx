...
The Tiles CLI includes DSRs ([dspy-rs](https://dsrs.herumbshandilya.com/)) to optimize `SYSTEM` prompts in Modelfiles automatically.
...
### `tiles optimize` details

The `tiles optimize` command refines the `SYSTEM` prompt in your Modelfile using DSRs (dspy-rs) and the COPRO optimizer.
