...
On the first device (online, with the peer DID):

```bash
$ tiles link create did:key:z6Mknxy...YWSmTcZA
```

For offline linking on the same local network, omit the DID:
...
**`tiles link create [PEER_DID]`**: Creates an authorization token for linking. Pass a peer DID for online UCAN linking; omit it for offline mDNS linking over the local network.
...
In Tiles `0.4.12`, inference controls lived under `tiles inference` with `run-background <bool>`. In `0.4.13`, they moved to `tiles server` with `start`, `stop`, and `daemon`.
