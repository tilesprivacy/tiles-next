...
<div id="remote-inference-nightly-tiles-remote" />
...
```bash
# Remote machine
tiles server daemon true # Starts the background inference server
...
**`tiles server daemon true`** keeps the inference server running in the background on the remote machine after the REPL exits.
