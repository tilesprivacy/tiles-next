...
<figure style={{ margin: '1.25rem 0 1.5rem 0' }}>
  <Image src="/inference-time-memory-module.png" alt="Memory agent revising a user prompt with minimal added context before model inference." width={1600} height={1257} style={{ borderRadius: '12px', border: '1px solid var(--nextra-border-color, rgba(0,0,0,0.1))', width: '100%', height: 'auto' }} />
  <figcaption style={{
    marginTop: '0.75rem',
    fontSize: '0.875rem',
    lineHeight: '1.4',
    ...
    User input prompt can be revised to add the minimally necessary context before request gets processed by the model
  </figcaption>
</figure>
...
Tiles `0.4.7` is the last release that includes this experiment; later versions removed this code path. To test it, download Tiles `0.4.7` from the [changelog release](/changelog#0.4.7) (or use any earlier release) and run `tiles run -m`.
