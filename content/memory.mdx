...
Our first experiment explores bundling a fine-tuned model to manage context and memories locally on-device using hyperlinked Markdown files. Currently, we use the [mem-agent](https://huggingface.co/driaforall/mem-agent) model from [Dria](https://dria.co/), based on [`qwen3-4B-thinking-2507`](https://github.com/QwenLM/Qwen3?tab=readme-ov-file#qwen3-2507), and are in the process of training our initial in-house memory models. For Tiles `0.4.7` and earlier, use `tiles run -m` to run this memory model. See the [`0.4.7` changelog release](/changelog#0.4.7). Read more about their training process and scaffolding design in their paper [here](https://huggingface.co/blog/driaforall/mem-agent-blog).
