...
export function ChangelogContent({ releases, error }: ChangelogContentProps) {
  ...
  const textColorSubtle = isDark ? 'text-white/40' : 'text-gray-400'
  const textColorBody = isDark ? 'text-white/70' : 'text-gray-600'
  const textColorBodyLight = isDark ? 'text-white/50' : 'text-gray-500'
  const textColorHeading = isDark ? 'text-white/90' : 'text-gray-900'
  const linkColor = isDark ? 'text-white/70 hover:text-white/90' : 'text-gray-700 hover:text-gray-900'
  const badgeBg = isDark ? 'bg-white/10' : 'bg-gray-100'
  const badgeText = isDark ? 'text-white/70' : 'text-gray-700'
  const badgeTextLight = isDark ? 'text-white/60' : 'text-gray-600'
  const timelineBg = isDark ? 'bg-white/20' : 'bg-gray-200'
  const dotBg = isDark ? 'bg-white' : 'bg-black'
  const dotRing = isDark ? 'ring-[#121212]' : 'ring-white'
  const bulletBg = isDark ? 'bg-white/40' : 'bg-gray-400'
  const bulletBgLight = isDark ? 'bg-white/30' : 'bg-gray-300'
