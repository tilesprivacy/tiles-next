...
import {
  ...
  SPONSOR_USDT_ETHERSCAN_URL,
} from "@/components/sponsor-usdt-donate-button"
...
function DonatePanel({ open }: { open: boolean }) {
  ...
  // Keep this component (and its amount/flow state) mounted across
  // close/reopen, matching the pre-split behavior where only the panel DOM
  // was conditional.
  if (!open) return null
...
export default function SponsorUsdtDonatePanel({ open }: { open: boolean }) {
  return (
    <WagmiProvider config={wagmiConfig}>
      <QueryClientProvider client={queryClient}>
        <DonatePanel open={open} />
      </QueryClientProvider>
    </WagmiProvider>
  )
}
