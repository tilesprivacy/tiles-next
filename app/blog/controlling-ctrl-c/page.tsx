  ...
  return (
    ...
    >
      ...
      <p>
        ...
        for agent harness. Since the REPL is written in Rust and Pi is in Typescript, we embed Pi Bun binary and use it
        via Pi&apos;s RPC mode, we spawn a headless Pi binary as a child process and communicate with it via{" "}
        <a href="https://en.wikipedia.org/wiki/Standard_streams" target="_blank" rel="noopener noreferrer">
          standard streams(stdin/stdout/stderr)
        </a>{" "}
        in json format. So the user input to the model and the streamed output from the model comes and goes through Pi.
        It sits between the REPL and the inference system.
      </p>
      ...
      <p>
        As with any LLM inference interface, Tiles REPL must stop the output streaming from the model as soon as user
      ...
      <p>
        ...
        <code>Err value: Os {"{ code: 32, kind: BrokenPipe, message: \"Broken pipe\" }"}</code> due to this users had to
        restart the REPL to continue. More details mentioned in this{" "}
      ...
      <p>
        ...
        reason the streams are connected via pipes rather than the child inheriting parent&apos;s streams (which is the
        default behavior). If the child inherited the parent&apos;s streams, communication would be messy and processes
        would have to filter what they need from the shared stream and this would introduce race conditions.
      </p>

      <p>
        When piped the parent can write to child&apos;s stdin, child will write to its own stdout, which then parent can
        read, thus having a clear separation of concerns. This is how we explicitly set the streams to be piped in Rust.
      </p>
    ...
    Command::new(pi_exec_path)
      ...
      <p>
        So when we press Ctrl-C, the SIGINT (Signal Interruption) event is being propagated to the child processes, and
        if they don&apos;t handle it then the default behavior is to exit. To monitor this, we can use following commands
        in unix systems.
      </p>
...
tiles   88068 tiles   17     PIPE  0x27aceab27161a82      16384   ->0x745d3d0e7a7709aa`} />
      ...
      <p>
        So when we press CTRL-C to stop a streaming response, the signal propagates to the Pi process and Pi exits. So
        when we try a new user prompt next time in REPL, unbeknownst to the REPL that such a process does not exist, it
        tries to still write to Pi&apos;s non-existent stdin, due to which we get broken pipe aka broken connection.
      </p>

      <h2>Letting them go..</h2>

      <p>
        Since by default child process is in same process group (pgid) as the parent, the SIGINT event is propagated to
        all the processes in the group. One way is to remove the child from the same group as the parent. This means they
        still have parent-child relationship but parent is no more in direct control of the children. In unix we can use{" "}
      ...
      <p>
        ...
        (where we lose safety assurance from the compiler) operation as setsid is only available in
        ...
        etc, ofcourse colored by unsafe keyword.
      </p>
    ...
    Command::new(pi_exec_path)
      ...
      <p>
        This resolves our broken pipe issue as the SIGINT is never reaching Pi process. But now we have another issue in
        hand, the model streaming is now non-stoppable via ctrl-c, the REPL is completely un-responsible during the
        entire time we are streaming the output from Pi&apos;s stdout!!. Its no longer respecting SIGINT!!
      </p>
      ...
      <p>
        ...
        which masks the SIGINT event. So we upgraded to latest version and YES now we are receiving SIGINT while model
        streaming, BUT the SIGINT exits the program all-together instead of stopping the stream and going back to user
        prompt waiting.
      </p>

      <p>
        This is in-fact expected as normally programs should handle the SIGINT themselves if they want to do cleanups,
        graceful shutdowns etc. So we use{" "}
        <a href="https://docs.rs/ctrlc/latest/ctrlc/" target="_blank" rel="noopener noreferrer">
          ctrlc
        </a>{" "}
        library to handle the SIGINT, which uses a dedicated thread for handling the event.
      </p>
...
while is_running.load(std::sync::atomic::Ordering::SeqCst) {
 ...
 // is_running to false
      ...
      <p>
        Once we refactored the codebase to be async, we started getting the SIGINT events in the handler which we set
        using ctrc library before and it is also not exiting the program too. Now all we have to do is to abort the Pi
        session by sending abort event to Pi and do the cleanup from our side.
      </p>
   ...
   if !is_running.load(std::sync::atomic::Ordering::SeqCst) {
      ...
      <p>
        The interesting thing now is that when we exit the main REPL program, Pi process also exit, which shouldn&apos;t
        be the case as both are now in different process group right? Could this be related to the pipes getting closed
        on one end?.
      </p>

      <p>
        Although this is fine for us, as we don&apos;t want Pi process to be a background daemon and being rogue, its
        important to understand what&apos;s happening under-the-hood as we also have a Tiles daemon process (which is a
        background headless Tiles http server) which is still alive even after the main REPL program closes as its
        supposed to be (this was also spawned in a different process group).
      </p>
...
\\-+= 88068 tiles target/debug/tiles
  ...
  \\--= 88099 tiles /Users/tiles/tiles/.tiles_dev/tiles/pi/pi --mode rpc`} />
      ...
      <p>
        Why dual behavior for same action. For that we can live debug Pi program using lldb (llvm debugger) to see what
        happens when Parent   exits. We will attach the Pi process to the lldb, add a breakpoint for SIGPIPE, then step
        through to see if Pi is handling SIGPIPE or not. The actions we do are commented with numbered index.
      </p>
...
Process 88099 exited with status = 0 (0x00000000)`} />
      ...
      <p>
        ...
        processes and their dynamic behavior on system signals with respect to its parent and finally to the problems
        caused by blocking IO in a UI and how async Rust can fix it.
      </p>
    </BlogPostContent>
  )
}
