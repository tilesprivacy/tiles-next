...
export async function GET(request: Request) {
  ...
  pushSection(lines, `Homepage (${baseUrl}/)`, [
    ...
    'Use cases: Individuals plan finances, track health notes, and keep a private journal, run models locally so that context stays on their machines, sync across devices, and share only when they choose. Coworking spaces turn shared compute workstations into private AI infrastructure so members connect from their own laptops, use the space local models, and collaborate while keeping identity and data theirs. Researchers review literature, analyze datasets, and launch AI agents in preconfigured, resumable sandboxes with the right research tools already installed.',
