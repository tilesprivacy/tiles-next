...
export async function GET(request: Request) {
  ...
  pushSection(lines, `Homepage (${baseUrl}/)`, [
    ...
    'Use cases: Individuals use Tiles for sensitive personal work like financial planning, health notes, and private journaling, run models locally so that context stays on their machines, sync across devices, and share only when they choose. Coworking spaces turn shared compute workstations into private AI infrastructure so members connect from their own laptops, use the space local models, and collaborate while keeping identity and data theirs. Researchers run literature reviews, analyze datasets, and collaborate on research without uploading unpublished findings to a hosted AI provider, ideal for sovereign compute and sensitive data.',
