...
export default async function BookLayout({
  ...
  const excludedRoutes = ['/', '/mission', '/download', '/explore', '/blog', '/releases', '/privacy', '/sub-processors', '/subprocessors', '/terms']
  ...
  // Resources is hidden from nav; restore by adding 'resources' back here and in book-mobile-nav / book-page-navigation / content/_meta.json.
  const desiredOrder = ['index', 'overview', 'manual', 'models', 'tilekit', 'security', 'community', 'opensource', 'licenses', 'finances']
  const hiddenBookSidebarPages = new Set(['research'])
  ...
  // Prefer the Book-native research chapter over the legacy `/research` redirect.
  const uniqueCardPageMap = cardOnlyPageMap.filter((item) => {
    const name = 'name' in item ? (item.name as string) : ''
    const route = 'route' in item ? (item.route as string) : ''
    return name !== 'research' || route !== '/research'
  })

  // Sort the pageMap according to the desired order
  const finalPageMap = uniqueCardPageMap.sort((a, b) => {
  ...
  return (
    <div className="relative flex min-h-screen flex-col bg-background" data-book-section>
      ...
      <BookTocScrollSync />
      <div className="pt-[var(--book-mobile-header-height)] lg:pt-[calc(7rem+env(safe-area-inset-top,0px))]">
