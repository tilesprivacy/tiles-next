...
- Bluesky rich previews for standard.site publications require `<link rel="site.standard.publication">` and `<link rel="site.standard.document">` in served HTML (render via `StandardSiteLinkTags` in Next.js blog layouts, not `sequoia inject` alone on Vercel), publication verification at `/.well-known/site.standard.publication/blog` when the publication URL is `/blog`, and raster `og:image` URLs (PNG or JPEG, not SVG) so Bluesky can show link preview thumbnails.
