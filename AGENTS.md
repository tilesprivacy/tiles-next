...
The website is undergoing a theme transition revamp in preparation for the beta release expected in the next few months.
...
Billing runs on [Polar](https://polar.sh), which acts as merchant of record. There are **two paid licenses**: **Tiles Plus** ($4 USD per user, per month) and **Tiles Pro** ($15 USD per user, per month), each backed by its own Polar product under the same organization. Entitlement is carried by a Polar-issued license key rather than any store on this site.
...
Checkout and webhooks are gated independently, so a missing signing secret cannot take down checkout and vice versa:
...
`Checkout()` instantiates a `Polar` client immediately, so build the handler **lazily inside** the configured check rather than at module scope:

```ts
export async function GET(request: NextRequest) {
  if (!isPolarCheckoutConfigured()) return polarNotConfiguredResponse()
...
Grant entitlements on `onSubscriptionActive` and revoke on `onSubscriptionRevoked`. Treat `onSubscriptionCanceled` as "ends at period end", not immediate loss of access. Handlers must be idempotent, since Polar retries on non-2xx. The handler bodies are still `TODO` stubs that only log.

### Customer portal

`content/licenses.mdx` and the `/pricing` FAQ link the hosted portal at `https://polar.sh/tilesprivacy/portal/` for license keys, renewals, billing, and purchase details. Keep using the hosted link unless an authenticated in-app portal is needed; only then add a `CustomerPortal({ accessToken, server, getCustomerId })` route, which requires resolving the signed-in customer.
...
Always gate these behind the matching `isPolar*Configured()` check, and never import them into client components.
...
- Keep the footer minimal but leave enough vertical rhythm in the copyright/credits/theme band on small screens so it does not read cramped; on narrow layouts, align the theme switcher and language switcher to the start of the row on mobile so they stay with left-stacked copy, language switcher labels should remain in their original language names, and changing language should not cause page position or layout shift on desktop or mobile. The subtext under the footer download CTA should stay aligned with the homepage hero platform line. Include a magenta `#CB30E0` trust line ("Done by an independent team devoted to user agency.") with a Lucide `Heart` icon (`fill-current`) directly above the copyright in the footer band, not isolated above the whole footer. On `/sponsor`, force dark tilesprivacy.org-style theming (black canvas, yellow `#f7ff61` accents via `lib/sponsor-page-theme.ts`) and hide the theme switcher; omit per-sponsor social icon links from current and past sponsor rows (avatar and name/handle only), describe the team as a "small independent team of three", and use sustainable maintainer wording ("let maintainers work on the project sustainably") instead of "keep all three contributors fed"; GitHub sponsors progress display falls back to `90%` via `SPONSORS_PROGRESS_PERCENT_FALLBACK` in `lib/sponsors-goal.ts`. Do not add an outgoing "ATproto stack" footer link; keep Status and the standard legal/nav links only. Footer and hero `TechAttribution` (`components/tech-attribution.tsx`) keeps Rust, MLX, Pi (linked to https://pi.dev, after MLX), ATproto, Iroh, and SQLCipher (monochrome padlock mark from `lib/sqlcipher-mark-path.ts`, after Iroh) with square logos at `h-[1em]`; the horizontal MLX wordmark uses `h-[0.62em]` so it matches their visual weight.
