...
6. When pricing stops being provisional, drop `PRICING_PAGE_STATUS_BADGE` and `PRICING_PAGE_STATUS_NOTE` from `lib/pricing-plans.ts` (and their render, plus `statusBadgeClass`, in `components/pricing-content.tsx`), refresh the "Is this pricing final?" FAQ, and update the Availability section in `content/licenses.mdx`.
